...
import made4mischief.astatine.client.screen.EntityTargetSelectorScreen;
...
public class EntityTargetComponent extends SettingComponent {
   ...
   public EntityTargetComponent(EntityTargetSetting setting, float x, float y, float width){
   ...
   public void render(GuiRenderContext context){
      if (this.isVisible()) {
         ...
         float textHeight = this.getY() + (this.getHeight() - context.textHeight() * 0.85F) / 2.0F;
         ...
         float textHeight2 = this.getY() + (this.getHeight() - context.textHeight() * 0.8F) / 2.0F;
         context.drawText(selectedCount, width + (70.0F - textWidth) / 2.0F, textHeight2, mouseY ? theme.accent() : theme.text(), true, 0.8F);
      }
   }
   ...
   protected boolean onMouseClicked(MouseClickEvent event){
      if (event.button() == MouseButton.LEFT && this.isInButtonArea(event.x(), event.y())) {
         ...
         client.setScreen(new EntityTargetSelectorScreen(screen, this.setting));
         event.consume(this);
         return true;
      } else {
         return false;
