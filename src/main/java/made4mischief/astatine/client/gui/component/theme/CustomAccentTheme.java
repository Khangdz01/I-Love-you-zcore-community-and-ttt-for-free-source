...
public final class CustomAccentTheme implements Theme {
   private final String name;
   private final Theme base = ThemeManager.DEFAULT;
   private int accent;
   private int secondaryColor;

   public CustomAccentTheme(String name, int accent, int secondary){
      this.name = name;
      this.setColors(accent, secondary);
   }

   public void setColors(int accent, int secondary){
      this.accent = ColorUtil.withAlpha(accent, 255);
      this.secondaryColor = ColorUtil.withAlpha(secondary, 255);
   }
