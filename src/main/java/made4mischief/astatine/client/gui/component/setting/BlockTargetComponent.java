...
import made4mischief.astatine.client.screen.BlockTargetSelectorScreen;
...
public final class BlockTargetComponent extends SettingComponent {
   ...
   public BlockTargetComponent(BlockTargetSetting setting, float x, float y, float width){
   ...
   protected boolean onMouseClicked(MouseClickEvent event){
      if (event.button() == MouseButton.LEFT && this.isInButtonArea(event.x(), event.y())) {
         ...
         client.setScreen(new BlockTargetSelectorScreen(screen, this.setting));
         event.consume(this);
         return true;
      } else {
         return false;
